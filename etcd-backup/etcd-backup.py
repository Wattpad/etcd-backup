...
S3_ENDPOINT_URL = os.getenv('ETCD_BACKUP_S3_ENDPOINT_URL')
...
def main():
    ...
    backup_interval = int(os.getenv('ETCD_BACKUP_INTERVAL_SEC', 60))
    s3_bucket = get_required_env_var('ETCD_BACKUP_S3_BUCKET')
    s3_prefix = get_required_env_var('ETCD_BACKUP_S3_PREFIX')
...
Required env vars:
  ETCD_BACKUP_S3_BUCKET: S3 bucket to upload backup tarball
  ETCD_BACKUP_S3_PREFIX: S3 prefix added to S3 object (useful for backup up multiple clusters into the same bucket)

Optional env vars:
  ETCD_DATA_DIR: etcd data directory  (default: /var/lib/etcd)
  ETCD_BACKUP_INTERVAL_SEC: number of seconds to wait between backup runs (default: 60)
""" % (sys.argv[0])
    print(message)
